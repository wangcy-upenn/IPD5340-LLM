...
// Order, for the page that compares them: lower means more comfortable.
// "unknown" is deliberately left out, so a vehicle SEPTA is not reporting on
// can never win a comparison and be recommended as the roomy option.
//   seats 0  <  few 1  <  standing 2  <  full 3
    ...
    const results = await Promise.all(
      ROUTES.map(async (route) => {
        try {
          ...
          // Every approaching vehicle goes on the board, not just the first of
          // each route - the whole point of prototype B is comparing them.
